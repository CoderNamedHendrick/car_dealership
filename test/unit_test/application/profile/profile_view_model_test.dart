...
void main() {
  ...
  group('Profile view model test suite', () {
    ...
    test('logout user success test', () async {
      ...
      verifyInOrder([
        ...
        () => listener(
            ...
            any(
                that: isA<ProfileUiState>()
                    .having((p0) => p0.uiState, 'current state is idle, since it clear the vm', UiState.idle)))
      ]);
    });
    ...
    test('fetch wishlist success test', () async {
      ...
      verifyInOrder([
        () => listener(null,
            currState.copyWith(wishlistUiState: currState.wishlistUiState.copyWith(uiState: UiState.idle))),
        () => listener(
            any(that: isA<ProfileUiState>()),
            any(
                that: isA<ProfileUiState>()
                    .having((p0) => p0.wishlistUiState.uiState, 'current state is loading', UiState.loading))),
        () => listener(
            any(that: isA<ProfileUiState>()),
            any(
                that: isA<ProfileUiState>()
                    .having((p0) => p0.wishlistUiState.uiState, 'current state is success', UiState.success)
                    .having(
    ...
    testWidgets('fetch wishlist failure test', (tester) async {
      ...
      verifyInOrder([
        ...
        () => listener(
            ...
            any(
                that: isA<ProfileUiState>()
                    .having((p0) => p0.wishlistUiState.uiState, 'current state is error', UiState.error)
