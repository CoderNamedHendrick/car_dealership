...
class ListingUiStateNotifier extends StateNotifier<ListingUiState> {
  ...
  Future<void> getListingReviews() async {
    ...
    state = state.copyWith(reviewsUiState: state.reviewsUiState.copyWith(uiState: UiState.loading));
    ...
    state = state.copyWith(
      reviewsUiState: results[0].fold(
        (left) => state.reviewsUiState.copyWith(uiState: UiState.error, error: left),
  ...
  Future<void> getIsSavedListing() async {
    ...
    state = state.copyWith(savedCarUiState: state.savedCarUiState.copyWith(uiState: UiState.loading));
    final result = await _carListingRepo.fetchSavedByUser(state.currentListing.id);

    state = state.copyWith(
      savedCarUiState: result.fold((left) => state.savedCarUiState.copyWith(uiState: UiState.error, error: left),
          (right) => state.savedCarUiState.copyWith(uiState: UiState.idle, isListingSaved: right)),
    );
  }
  ...
  Future<void> checkIfNegotiationAvailable() async {
    ...
    state = state.copyWith(contactSellerUiState: state.contactSellerUiState.copyWith(uiState: UiState.loading));
    ...
    state = state.copyWith(
      contactSellerUiState: result.fold(
          (left) => state.contactSellerUiState
              .copyWith(uiState: UiState.error, error: left, isOngoingNegotiation: false),
          (right) => state.contactSellerUiState.copyWith(uiState: UiState.success, isOngoingNegotiation: right)),
    );
  }
  ...
  void toggleSaveListing() async {
    ...
    if (state.savedCarUiState.uiState == UiState.loading) return; // don't perform action when loading

    await launch(state.savedCarUiState.reference, (model) async {
      state = state.copyWith(
        savedCarUiState: model.emit(state.savedCarUiState.copyWith(uiState: UiState.loading)),
      );
      ...
      state = state.copyWith(
        savedCarUiState: result.fold(
          (left) => model.emit(
            state.savedCarUiState.copyWith(uiState: UiState.error, error: left, isListingSaved: false),
          ),
          (right) => model.emit(state.savedCarUiState.copyWith(uiState: UiState.success)),
        ),
      );
    ...
    state = state.copyWith(savedCarUiState: state.savedCarUiState.copyWith(uiState: UiState.idle));
  }

  void ratePurchase(int rating) async {
    await launch(state.purchaseRatingUiState.reference, (model) async {
      state = state.copyWith(
          purchaseRatingUiState: model.emit(state.purchaseRatingUiState.copyWith(uiState: UiState.loading)));
      final result =
      ...
      state = state.copyWith(
        purchaseRatingUiState: result.fold(
            (left) => model.emit(state.purchaseRatingUiState.copyWith(uiState: UiState.error, error: left)),
            (right) => model.emit(state.purchaseRatingUiState.copyWith(uiState: UiState.success))),
      );
    });
  }
}
