...
Future<RangeValues?> showRangeFilteringOptions(
  ...
  int? divisions,
  required RangeRenderCallback rangeRenderCallback,
}) async {
    ...
    builder: (_) => FilterDialogBackgroundWrapper(
      child: FilterPage(
        child: FilterSliderScreen(
          ...
          currentValue: range,
          divisions: divisions,
          rangeRender: rangeRenderCallback,
        ),
      ),
    ),
  );
              ...
              Map info? => MaterialPageRoute<RangeValues?>(
                  builder: (_) => FilterPage(
                    child: FilterSliderScreen(
                      ...
                      rangeRender:
                          info['rangeRenderCallback'] as RangeRenderCallback,
                    ),
                  ),
                ),
...
class FilterMenu extends ConsumerWidget with MIntl {
  const FilterMenu({super.key});

  @override
  Widget build(BuildContext context, ref) {
    final filter =
        ref.watch(filterStateNotifierProvider.select((value) => value.filter));
    return Padding(
        ...
        children: [
          ...
          FilterListTile(
            ...
            subtitle: switch ((filter.minPrice, filter.maxPrice)) {
              (final minPrice?, null) => FilterSubtitle(
                  'price >= ${currentFormatWithoutDecimals.format(minPrice)}'),
              (null, final maxPrice?) => FilterSubtitle(
                  'price <= ${currentFormatWithoutDecimals.format(maxPrice)}'),
              (final minPrice?, final maxPrice) => FilterSubtitle(
                  ' ${currentFormatWithoutDecimals.format(minPrice)} <= price <= ${currentFormatWithoutDecimals.format(maxPrice)}'),
              _ => null,
            },
            onTap: () async {
              final result =
                  await Navigator.of(context).pushNamed<RangeValues?>(
                FilterSliderScreen.route,
                arguments: {
                  'title': 'Price',
                  'min': 1000.0,
                  'max': 100000.0,
                  'range': RangeValues(
                      filter.minPrice ?? 1000, filter.maxPrice ?? 100000),
                  'rangeRenderCallback': (value) {
                    return 'Price range: ${currentFormatWithoutDecimals.format(value.start)} <= ${currentFormatWithoutDecimals.format(value.end)}';
                  },
                },
              );
            ...
            onTap: () async {
              final result =
                ...
                arguments: {
                  ...
                  'max': 2023.0,
                  'divisions': 13,
                  'range': RangeValues(filter.minYear?.toDouble() ?? 2010,
                      filter.maxYear?.toDouble() ?? 2023),
                  'rangeRenderCallback': (value) {
                    return 'Manufacture year range: ${value.start.toInt()} <= ${value.end.toInt()}';
                  },
                },
              );
              ...
              if (!context.mounted) return;
              if (result == null) return;
              ref.read(filterStateNotifierProvider.notifier).updateYear(
          ...
          FilterListTile(
            ...
            onTap: () async {
              ...
              if (!context.mounted) return;
              if (result == null) return;
              ref.read(filterStateNotifierProvider.notifier).updateMileage(
          ...
          FilterListTile(
            label: 'Color',
            clearFilterOnTap:
                ref.read(filterStateNotifierProvider.notifier).clearColorFilter,
          ...
          FilterListTile(
            ...
            onTap: () async {
              ...
              ref
                  .read(filterStateNotifierProvider.notifier)
                  .updateFuelType(result.fuelType);
            },
          ),
          FilterListTile(
            label: 'Availability',
